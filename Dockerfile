...
COPY --chown=appuser:appuser services/watcher/ .
...
COPY --chown=appuser:appuser services/ner/ .
EXPOSE 8000
USER appuser
...
COPY --chown=appuser:appuser services/rss/ .
# Note: config.yaml is usually mounted via volume, but we copy a default here
COPY --chown=appuser:appuser services/rss/config.yaml ./config.yaml
RUN chmod +x entrypoint.sh
EXPOSE 8000
USER appuser
ENTRYPOINT ["./entrypoint.sh"]
...
COPY --chown=appuser:appuser services/harvester/ .
USER appuser
...
RUN useradd --create-home --shell /usr/sbin/nologin appuser && chown -R appuser:appuser /usr/src/app
USER appuser
CMD ["bash"]
